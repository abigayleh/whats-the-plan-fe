...
// The trailing hint on a row: a completed trip shows the year it ended, an unplanned one
// its expected length. A dated, still-active trip shows nothing.
export function itineraryMeta(itinerary, completed) {
  if (completed) {
    const ended = itinerary.endDate ?? itinerary.startDate;
    return ended ? `(${ended.getFullYear()})` : null;
  }
